...
export const DisabledEgresses = () => {
	...
	}, [filters, trigger, isUpdated]);

    const handleEnable = (id: number) => {
        api.post(`egress/restore/${id}`)
        .then(() => {
            setIsUpdated(true)
            return toast.success('Usuário habilitado com sucesso!')
        })
        .catch(() => {
            return toast.error('Erro ao habilitar usuário!')
        })
    }
